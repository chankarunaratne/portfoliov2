...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function isMobileDevice() {
   ...
   function initializeMobileVideoThumbnail() {
     if (isMobileDevice() && videoThumbnail) {
       // Create Loom GIF thumbnail for mobile
       const loomGifHTML = `
         <a href="${loomVideoUrl}" target="_blank" rel="noopener noreferrer" style="display: block; width: 100%; height: 100%; position: relative; text-decoration: none;">
           <img 
             src="https://cdn.loom.com/sessions/thumbnails/${loomVideoId}-with-play.gif" 
             alt="Chan's Video Intro - Click to watch on Loom"
       ...
       // Remove click event handlers for mobile since we're using direct link
       const newVideoThumbnail = videoThumbnail.cloneNode(true);
       videoThumbnail.parentNode.replaceChild(newVideoThumbnail, videoThumbnail);
       
       // Update button text for mobile
       if (watchBtn) {
         watchBtn.textContent = 'Watch on Loom';
         watchBtn.addEventListener('click', function(e) {
           e.preventDefault();
           window.open(loomVideoUrl, '_blank', 'noopener,noreferrer');
         });
       }
  ...
  // Only add desktop modal event handlers if not on mobile
  if (!isMobile) {
    ...
    // Video thumbnail click (desktop only)
    if (videoThumbnail) {
  ...
  // Modal close event listeners (only for desktop)
  if (!isMobile) {
    if (videoModalClose) {
